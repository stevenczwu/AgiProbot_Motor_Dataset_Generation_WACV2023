    ...
    if x.shape[1] == 13:
        for i in range(x.shape[0]):
            if x[i][8] == 808464432.0:
                ...
                x[i][8] = int(6)
            # elif x[i][8] == 926365495.0: //bolt
            #     x[i][8] = int(7)
            elif x[i][8] == 909522486.0+16843009.0:
                x[i][8]=int(8)
            elif x[i][8] == 909522486.0+ 16843009.0*2:
                x[i][8]=int[9]
    else:
...
def cut(data_to_cut, noise, camera_position_now,cut_xyz):
    if cut_xyz:
...
def cuboid_choice(save_dir, numpy_name, cuboid_format, filtered, cam_info, noise=True,cut_xyz=1):
    filtered2 = cut(data_to_cut=filtered, noise=noise, camera_position_now=cam_info,cut_xyz=cut_xyz)
...
def scan_cut(save_dir, Motor_type, sequence_Motor, cam_info, cuboid_format='npy',cut_xyz=1):
    bpy.ops.object.select_all(action='DESELECT')
    bpy.context.scene.camera = bpy.data.objects['Camera']
    bpy.data.objects['Camera'].select = True
    bpy.context.scene.objects.active = bpy.data.objects['Camera']
    scanner = bpy.data.objects['Camera']
    sigma1 = random.uniform(0.002, 0.006)
    ...
    numpy_name = 'Training_' + Motor_type + '_' + sequence_Motor + '_augmentation'
    ...
    cuboid_choice(save_dir, numpy_name, cuboid_format, filtered, cam_info, noise=False,cut_xyz=cut_xyz)
...
def parse_opt():
    ...
    parser.add_argument(
        '-i', '--input', dest='motor_path', metavar='FILE', default="/home/bi/study/thesis/data/Motors",help="path to the motor mesh models dir"
    )
    parser.add_argument(
        '-o', '--output', dest='save_path', metavar='FILE',default="/home/bi/study/thesis/data/synthetic/Dataset3_addition", help="path to the output dir"
    )
    parser.add_argument(
        '-clp', dest='clamping_path', metavar='FILE', default="/home/bi/study/thesis/Motor_Datasets_Generation-master/clamping_system",
    ...
    parser.add_argument(
        '-n', '--num', type=int,default=1000, help='total generation number'
    )
...
def main(args):
    ...
    for motor_type in list_types_motor:
        ...
        for dirs in List_motor:
            ...
            random_cam_info = random_CameraPosition(radius_camera=random.uniform(2.85, 3.23))
            if motor_deflection_switch:
                motor_deflection = np.random.uniform([-15 * p, -5 * p, -5 * p], [15 * p, 5 * p, 5 * p])
            else:
                motor_deflection = [0,0,0]
            ...
            scan_cut(save_dir=save_dir, Motor_type=motor_type, sequence_Motor=k[1], cam_info=random_cam_info,
                     motor_info=motor_deflection, cuboid_format=args.cuboid_file_format,cut_xyz=cut_xyz)
            flag += 1
